...
class _EditDiaryEntryScreenState extends State<EditDiaryEntryScreen> {
  ...
  late List<DiaryExpense> _expenses;
  ...
  void initState() {
    ...
    _photos = List.from(widget.entry.photos);
    _expenses = List.from(widget.entry.expenses);
  }
  ...
  void _deleteImage(int index) {
    ...
    setState(() {
      ...
      _deletedImagePaths.add(photo.url);
    });
  }
  ...
  Future<void> _updateDiary() async {
    ...
    try {
      ...
      await _diaryRepository.updateDiaryEntry(updatedEntry);
      ...
      appLogger.i('다이어리 업데이트 완료: ${updatedEntry.id}');

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('다이어리가 수정되었습니다')),
        );
        Navigator.pop(context, true);
      }
    } catch (e) {
      appLogger.e('다이어리 업데이트 실패', error: e);
